...
export default function RecuperarPassword() {
  ...
  const handleRecuperar = async (e) => {
    ...
    if (!emailRegex.test(email)) {
      setError(
        "Por favor, ingresa un formato de correo válido (ejemplo@empresa.com).",
      );
      return;
    }
    ...
    try {
      ...
      if (!usuarioSnap.exists()) {
        setError(
          "No encontramos ningún usuario registrado con este correo en nuestro sistema.",
        );
        setLoading(false);
        return;
      }
      ...
      setMensaje(
        "¡Listo! Revisa tu bandeja de entrada o la carpeta de spam para restablecer tu contraseña.",
      );
      setEmail("");
    } catch (err) {
      ...
      if (err.code === "auth/invalid-email") {
        ...
        setError(
          "Has hecho demasiados intentos. Por favor, intenta más tarde.",
        );
      } else {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col items-center justify-center p-4 relative">
      ...
      <div className="w-32 h-32 bg-black rounded-full flex items-center justify-center -mb-8 relative z-10 shadow-sm border-4 border-white">
        <img
          src="/EmailRecovery.png"
          alt="Recuperar"
          className="w-[6rem] h-auto -mb-4 relative z-10"
        />
      </div>
