...
export default function Gest_Usuarios() {
  ...
  const [popoverRolActivo, setPopoverRolActivo] = useState(null);
  ...
  const cargarRoles = async () => {
    try {
      ...
      const rolesFiltrados = querySnapshot.docs
        ...
        .filter(
          (rolId) => rolId !== "USUARIO/EMPLEADO" && rolId !== "EMPLEADO",
        );
  ...
  const isFiltroActivo =
    filtrosAplicados.rol !== "" || filtrosAplicados.estado !== "";
  ...
  const abrirModalSelectorRol = (user) => {
    setUsuarioSeleccionado(user);
    const rolesArray = Array.isArray(user.rol)
      ? user.rol
      : user.rol
  ...
  const confirmarCambioRol = async () => {
    ...
    try {
      ...
      setUsuarios(
        usuarios.map((u) =>
          u.id === usuarioSeleccionado.id ? { ...u, rol: rolesFinales } : u,
        ),
      );
      cerrarModalRol();
    } catch (error) {
  ...
  const usuariosFiltrados = usuarios.filter((user) => {
    ...
    const matchEstado =
      ...
      user.estado?.toUpperCase() === filtrosAplicados.estado;
    return matchBusqueda && matchRol && matchEstado;
  });

  const totalPaginas =
    Math.ceil(usuariosFiltrados.length / itemsPorPagina) || 1;
  ...
  const TableSkeleton = () => (
    <div className="w-full animate-pulse">
      ...
      {[...Array(6)].map((_, i) => (
        <div
          key={i}
          className="flex items-center justify-between py-5 px-4 border-b border-gray-100"
        >
          <div className="w-[15%]">
    ...
    <div className="p-4 md:p-8 max-w-[1600px] mx-auto">
      <div className="mb-8">
        ...
        <p className="text-[13px] text-gray-500 mt-1 font-medium">
          Visualización y gestión del estado de la cuenta de cada usuario
        </p>
      </div>

      <div className="bg-white p-6 rounded-[1.5rem] shadow-[0_2px_20px_rgb(0,0,0,0.02)] border border-gray-200 relative z-10">
          ...
          <div className="flex gap-3 w-full md:w-auto relative">
            <button
              ...
              className={`border text-[11px] font-bold px-4 py-2.5 rounded-xl flex items-center justify-center gap-2 transition-colors w-full md:w-auto
                ${isFiltroActivo ? "bg-purple-50 border-purple-200 text-[#7C3AED] hover:bg-purple-100" : "bg-white border-gray-200 text-gray-600 hover:bg-gray-50"}
              `}
            >
              {isFiltroActivo ? (
                <XCircle className="w-4 h-4" />
            ...
            {mostrarFiltros && (
              <div
                ref={filtroRef}
                className="absolute top-12 right-0 md:right-36 w-64 bg-white rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-gray-100 z-50 p-4"
              >
                ...
                <div className="mb-4">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setFiltrosTemp({ ...filtrosTemp, rol: e.target.value })
                    }
            ...
            <Link
              to="/usuarios/nuevo"
              className="bg-[#020817] text-white text-[11px] font-bold px-5 py-2.5 rounded-xl flex items-center justify-center gap-2 hover:bg-black transition-colors shadow-md w-full md:w-auto uppercase tracking-widest"
            >
              Asignar Usuario
            </Link>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-[#F8F9FF] rounded-xl">
                  ...
                  <th className="py-3 px-4 text-[10px] font-bold text-gray-500 uppercase tracking-widest text-center">
                    Roles Asignados
                  </th>
                  <th className="py-3 px-4 text-[10px] font-bold text-gray-500 uppercase tracking-widest text-center">
                    Estado
                ...
                {usuariosPaginados.map((user) => {
                  ...
                  return (
                    ...
                    >
                        ...
                        <div className="relative flex items-center justify-center gap-1.5">
                          {rolesArray.length > 1 ? (
                            <div className="flex items-center gap-1.5">
                              <span
                                className={`text-[9px] font-bold px-3 py-1 rounded-full uppercase tracking-widest shadow-sm ${rolesArray[0] === "EMPLEADO" ? "bg-gray-200 text-gray-600" : "bg-[#020817] text-white"}`}
                              >
                                {rolesArray[0]}
                              </span>
                              <button
                                onClick={() =>
                                  setPopoverRolActivo(
                                    popoverRolActivo === user.id
                                      ? null
                                      : user.id,
                                  )
                                }
                                className="text-[9px] font-black bg-purple-100 text-[#7C3AED] px-2 py-1 rounded-full hover:bg-purple-200 transition-colors shadow-sm"
                              >
                                +{rolesArray.length - 1}
                              </button>
                              {popoverRolActivo === user.id && (
                                <div
                                  ref={popoverRef}
                                  className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 w-40 bg-white rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-gray-100 z-[60] p-3 animate-in fade-in slide-in-from-bottom-1"
                                >
                                  <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-2 border-b border-gray-50 pb-1">
                                    Lista de Roles
                                  </p>
                                  <div className="flex flex-col gap-1.5">
                                    {rolesArray.map((r, i) => (
                                      <span
                                        key={i}
                                        className={`text-[9px] font-bold px-2 py-1 rounded-lg uppercase tracking-widest text-center ${r === "EMPLEADO" ? "bg-gray-100 text-gray-500" : "bg-[#020817] text-white"}`}
                                      >
                                        {r}
                                      </span>
                                    ))}
                      ...
                      <td className="py-5 px-4 text-center">
                        <span
                          className={`text-[9px] font-bold px-4 py-1.5 rounded-full uppercase tracking-widest inline-block ${activo ? "bg-green-50 text-green-600" : "bg-red-50 text-red-500"}`}
                        >
                          {user.estado}
                        </span>
                      </td>
                      <td className="py-5 px-4 text-center relative">
                        ...
                        {menuActivo === user.id && (
                          <div
                            ref={menuRef}
                            className="absolute right-12 top-10 w-48 bg-white rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-gray-200 z-50 text-left py-2"
                          >
        ...
        {!cargando && usuariosFiltrados.length > 0 && (
          <div className="flex flex-col sm:flex-row items-center justify-between mt-6 pt-4 border-t border-gray-50 gap-4">
            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">
              ...
              {Math.min(startIndex + itemsPorPagina, usuariosFiltrados.length)}{" "}
              de {usuariosFiltrados.length} Resultados
            </p>
      ...
      {modalConfirmacion && usuarioSeleccionado && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#020817]/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-[1.5rem] shadow-2xl w-full max-w-md overflow-hidden transform transition-all">
            <div
              className={`${usuarioSeleccionado.estado === "ACTIVO" ? "bg-red-50 border-red-100" : "bg-green-50 border-green-100"} p-6 flex flex-col items-center border-b relative`}
            >
              <button
                onClick={cerrarModalEstado}
                className={`absolute top-4 right-4 ${usuarioSeleccionado.estado === "ACTIVO" ? "text-red-400 hover:text-red-600" : "text-green-400 hover:text-green-600"} transition-colors`}
              >
                <X className="w-5 h-5" />
              </button>
              <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center shadow-sm mb-4">
                <AlertCircle
                  className={`w-8 h-8 ${usuarioSeleccionado.estado === "ACTIVO" ? "text-red-500" : "text-green-500"}`}
                />
              </div>
              <h3
                className={`text-lg font-extrabold text-center ${usuarioSeleccionado.estado === "ACTIVO" ? "text-red-600" : "text-green-600"}`}
              >
                {usuarioSeleccionado.estado === "ACTIVO"
                  ? "Suspender"
                  : "Activar"}{" "}
                acceso a {usuarioSeleccionado.nombre}?
              </h3>
            </div>
            <div className="p-6">
              ...
              <input
                ...
                className={`w-full text-center bg-[#F8F9FF] border text-sm font-bold px-4 py-3 rounded-xl focus:outline-none focus:ring-2 transition-all ${usuarioSeleccionado.estado === "ACTIVO" ? "border-gray-200 focus:ring-red-500/20 focus:border-red-500" : "border-gray-200 focus:ring-green-500/20 focus:border-green-500"}`}
              />
              <div className="flex items-center gap-3 mt-6">
                ...
                <button
                  ...
                  disabled={
                    inputConfirmacion !== usuarioSeleccionado.id ||
                    procesandoEstado
                  }
                  className={`flex-1 text-white text-[11px] font-bold py-3 rounded-xl shadow-md transition-all tracking-widest uppercase ${inputConfirmacion !== usuarioSeleccionado.id || procesandoEstado ? "bg-gray-300 cursor-not-allowed opacity-70" : usuarioSeleccionado.estado === "ACTIVO" ? "bg-red-500 hover:bg-red-600" : "bg-green-500 hover:bg-green-600"}`}
                >
                  {procesandoEstado
                    ? "Procesando..."
                    : usuarioSeleccionado.estado === "ACTIVO"
          ...
          <div className="bg-white rounded-[1.5rem] shadow-2xl w-full max-w-md overflow-hidden transform transition-all">
            <div className="bg-purple-50 p-6 flex flex-col items-center border-b border-purple-100 relative">
              ...
              <h3 className="text-lg font-extrabold text-purple-700 text-center leading-tight">
                Asignar Rol Administrativo a<br />
            ...
            <div className="p-6">
                  ...
                  {rolesDb.length === 0 ? (
                    ...
                    rolesDb.map((r) => (
                      <button
                        ...
                        className={`w-full px-4 py-3 text-xs font-bold rounded-xl transition-all border text-left flex items-center justify-between ${rolDestino === r ? "bg-purple-50 text-purple-700 border-purple-300 shadow-sm" : "bg-white text-gray-500 border-gray-200 hover:bg-gray-50"}`}
                      >
                        {r}{" "}
                        {rolDestino === r && (
              ...
              <div className="flex items-center gap-3 mt-6">
                ...
                <button
                  ...
                  className={`flex-1 text-white text-[11px] font-bold py-3 rounded-xl shadow-md transition-all tracking-widest uppercase ${inputConfirmacion !== usuarioSeleccionado.id || !rolDestino || procesandoRol ? "bg-gray-300 cursor-not-allowed opacity-70" : "bg-purple-600 hover:bg-purple-700"}`}
                >
