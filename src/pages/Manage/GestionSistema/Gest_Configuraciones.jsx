...
export default function Gest_Configuraciones() {
  ...
  const cargarDatos = async () => {
    ...
    try {
      ...
      setPorcentajeInteres(
        String(Math.round((config.porcentajeInteres || 0.15) * 100)),
      );
  ...
  const handleAgregarPlazo = async () => {
    ...
    if (plazos.some((p) => p.plazoMeses === meses))
      return alert("Ese plazo ya existe.");
    setAgregandoPlazo(true);
    try {
      const nuevo = await agregarPlazo(meses);
      setPlazos((prev) =>
        [...prev, nuevo].sort((a, b) => a.plazoMeses - b.plazoMeses),
      );
      setNuevoPlazoMeses("");
    } catch (error) {
  ...
  const confirmarEjecucionManual = async () => {
    if (ejecutandoManual) return;
    ...
    try {
      ...
      setTimeout(() => setModalDeducciones(false), 2000);
    } catch (e) {
      console.error("Error en ejecución manual:", e);
      setError(
        e.details?.message ||
          e.message ||
          "Error desconocido al procesar cuotas",
      );
    } finally {
              ...
              <div className="grid grid-cols-7 gap-y-1 gap-x-1 text-center">
                {diasGrid.map((day, index) => (
                  <button
                    ...
                    disabled={day === null}
                    onClick={() => {
                      if (day !== null) setDiaFechaCobro(String(day));
                    }}
                    className={`h-8 w-full rounded-lg text-[12px] font-bold transition-all ${
                      day === null
                        ? "opacity-0 cursor-default"
                        : String(day) === String(diaFechaCobro)
                          ? "bg-[#7C3AED] text-white shadow-md shadow-[#7C3AED]/30 scale-105"
          ...
          <div className="bg-white rounded-[1.5rem] shadow-2xl w-full max-w-md overflow-hidden transform transition-all border border-gray-100">
            <div className="bg-orange-50/80 p-6 flex flex-col items-center border-b border-orange-100 relative">
              ...
              <p className="text-[12px] font-medium text-orange-600 mt-2 text-center px-4 leading-relaxed">
                Esta acción procesará todos los cobros programados de forma
                inmediata. Solo usa esto si el proceso automático falló.
              </p>
            </div>
            <div className="p-6">
              <div className="mb-5">
