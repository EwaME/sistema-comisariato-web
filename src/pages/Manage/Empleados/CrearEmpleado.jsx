...
export default function CrearEmpleado() {
    ...
    const cargarListasDinamicas = async () => {
      try {
        const [deptSnap, cargosSnap] = await Promise.all([
          ...
          getDocs(
            query(collection(db, "cargos"), where("estado", "==", "ACTIVO")),
          ),
        ]);
    ...
    const inicializarFormulario = async () => {
      ...
      if (isEdit) {
        try {
          ...
          if (
            empleadoAEditar.salario !== undefined &&
            empleadoAEditar.salario !== null
          ) {
            empleadoAEditar.salario = parseFloat(
              empleadoAEditar.salario,
            ).toFixed(2);
          }
  ...
  const formatearDNI = (valor) => {
    ...
    if (soloNumeros.length > 8)
      formateado = `${soloNumeros.slice(0, 4)}-${soloNumeros.slice(4, 8)}-${soloNumeros.slice(8, 13)}`;
    return formateado;
  };
  ...
  const formatearTelefono = (valor) => {
    ...
    if (soloNumeros.length > 0) {
      if (soloNumeros.length > 4)
        formateado += `${soloNumeros.slice(0, 4)}-${soloNumeros.slice(4, 8)}`;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const limiteBase = salarioBase * 0.3;
    ...
    <div className="p-4 max-w-[1200px] mx-auto bg-[#F8F9FF] min-h-screen">
      ...
      <div className="mb-8">
        ...
        <p className="text-[13px] text-gray-500 mt-1 font-medium">
          {isEdit
            ? "Actualiza la información del expediente del empleado."
      ...
      <form
        ...
        className="bg-white p-6 md:p-10 rounded-[1.5rem] shadow-[0_2px_20px_rgb(0,0,0,0.02)] border border-gray-50"
      >
        <div className="flex items-center gap-6 mb-10">
          ...
          <div>
            <h4 className="font-bold text-[#020817] mb-1">
              Fotografía de perfil
            </h4>
            <p className="text-xs text-gray-400">
              Clickea la imagen para subir (JPG/PNG. Máx 5MB).
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div>
            <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">
              Código de Empleado (Auto)
            </label>
            <input
              readOnly
              value={isEdit ? id : formData.empleadoId}
          ...
          <div>
            ...
            >
              ...
              {cargos.map((car) => (
                <option key={car.id} value={car.nombre}>
                  {car.nombre}
                </option>
              ))}
            </select>
        ...
        <div className="flex items-center justify-end gap-3 pt-6 border-t border-gray-50">
          <button
            type="button"
            onClick={() => navigate("/empleados")}
            className="bg-white border border-gray-200 text-gray-600 text-[11px] font-bold px-6 py-3 rounded-xl hover:bg-gray-50 transition-colors tracking-widest uppercase"
          >
            Cancelar
          </button>
          <button
            type="submit"
            disabled={
              cargando || (!isEdit && formData.empleadoId.includes("Cargando"))
            }
            className={`text-white text-[11px] font-bold px-8 py-3 rounded-xl shadow-md transition-colors tracking-widest uppercase ${cargando || (!isEdit && formData.empleadoId.includes("Cargando")) ? "bg-gray-400 cursor-not-allowed" : "bg-[#020817] hover:bg-black"}`}
          >
            {cargando
              ? "Guardando..."
              : isEdit
