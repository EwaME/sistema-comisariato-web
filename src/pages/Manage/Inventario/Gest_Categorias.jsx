...
import { Search, Plus, ImageIcon, Loader2, MoreVertical, X, AlertCircle, Package, ChevronLeft, ChevronRight } from 'lucide-react';
import { obtenerCategoriasConConteo, cambiarEstadoCategoria } from '../../../services/categoriasService';
...
export default function Gest_Categorias() {
    ...
    const registrosActuales = filtradas.slice(indicePrimerRegistro, indiceUltimoRegistro);
                    ...
                    return (
                        <div key={cat.id} className="bg-white rounded-[2rem] p-6 border border-gray-50 shadow-sm hover:shadow-md transition-all group relative flex flex-col items-center">
                            ...
                            <div className="text-center">
                                <span className={`text-[9px] font-black uppercase px-3 py-1 rounded-full ${esActivo ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'}`}>
            ...
            {!cargando && filtradas.length > 0 && (
                <div className="flex flex-col sm:flex-row items-center justify-between mt-4 pt-6 border-t border-gray-100 gap-4 bg-white p-4 rounded-2xl shadow-sm">
                    <p className="text-[11px] font-bold text-gray-400 uppercase tracking-widest">
                        Mostrando {indicePrimerRegistro + 1} - {Math.min(indiceUltimoRegistro, filtradas.length)} de {filtradas.length} categorías
                    </p>
            ...
            {modalDesactivar && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#020817]/40 backdrop-blur-sm p-4">
                    <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-md overflow-hidden transform transition-all animate-in zoom-in-95 duration-200">
                        <div className={`p-6 flex flex-col items-center border-b relative ${categoriaSeleccionada?.estado?.toUpperCase() === 'ACTIVO' ? 'bg-red-50 border-red-100' : 'bg-green-50 border-green-100'}`}>
                            ...
                            <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center shadow-sm mb-4">
                                <AlertCircle className={`w-8 h-8 ${categoriaSeleccionada?.estado?.toUpperCase() === 'ACTIVO' ? 'text-red-500' : 'text-green-500'}`} />
                            </div>
                            <h3 className={`text-lg font-extrabold text-center ${categoriaSeleccionada?.estado?.toUpperCase() === 'ACTIVO' ? 'text-red-600' : 'text-green-600'}`}>
                                ¿{categoriaSeleccionada?.estado?.toUpperCase() === 'ACTIVO' ? 'Desactivar' : 'Activar'} categoría {categoriaSeleccionada?.nombre}?
                            </h3>
                                ...
                                <button 
                                    ...
                                    className={`flex-1 text-white text-[11px] font-bold py-3.5 rounded-xl shadow-md transition-all tracking-widest uppercase
                                        ${inputConfirmacion !== categoriaSeleccionada?.categoriaId || procesando
                                            ...
                                            : categoriaSeleccionada?.estado?.toUpperCase() === 'ACTIVO' ? 'bg-red-500 hover:bg-red-600' : 'bg-green-500 hover:bg-green-600'
                                        }`}
                                >
                                    {procesando ? 'Procesando...' : `Sí, ${categoriaSeleccionada?.estado?.toUpperCase() === 'ACTIVO' ? 'Desactivar' : 'Activar'}`}
                                </button>
                            </div>
                        </div>
                    </div>
