...
export default function Gest_Categorias() {
  ...
  const cargarCategorias = async () => {
    try {
      setCargando(true);
      const data = await obtenerCategoriasConConteo();
      setCategorias(data);
    } catch (error) {
      console.error(error);
    } finally {
  ...
  const handleDesactivar = async () => {
    ...
    setProcesando(true);
    try {
      const esActivo = categoriaSeleccionada.estado?.toUpperCase() === "ACTIVO";
      const nuevoEstado = esActivo ? "INACTIVO" : "ACTIVO";

      await cambiarEstadoCategoria(categoriaSeleccionada.id, nuevoEstado);
  ...
  const filtradas = categorias.filter(
    (c) =>
      ...
      c.categoriaId?.toLowerCase().includes(busqueda.toLowerCase()),
  );

  const indiceUltimoRegistro = paginaActual * registrosPorPagina;
  const indicePrimerRegistro = indiceUltimoRegistro - registrosPorPagina;
  const registrosActuales = filtradas.slice(
  ...
  if (cargando)
    return (
      <div className="flex justify-center items-center min-h-screen">
        <Loader2 className="animate-spin text-[#7C3AED] w-10 h-10" />
      </div>
    );
  ...
  return (
    <div className="p-8 bg-[#F8F9FF] min-h-screen">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-500 text-sm font-medium">
            Control de categorías de los productos. Campo clave para la
            navegación en el portal móvil
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 mb-8">
        {registrosActuales.map((cat) => {
          const esActivo = cat.estado?.toUpperCase() === "ACTIVO";

          return (
            <div
              key={cat.id}
              className="bg-white rounded-[1.5rem] p-6 border border-gray-200 shadow-sm hover:shadow-md transition-all group relative flex flex-col items-center"
            >
              <div className="absolute top-4 right-4">
                ...
                {menuActivo === cat.id && (
                  <div
                    ref={menuRef}
                    className="absolute left-0 mt-2 w-40 bg-white rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-gray-100 z-50 text-left overflow-hidden"
                  >
              ...
              <div className="w-32 h-32 mb-4 bg-black rounded-full  flex items-center justify-center border-4 border-white shadow-inner">
                {cat.imagenUrl ? (
                  <img
                    src={cat.imagenUrl}
                    alt={cat.nombre}
                    className="w-35 h-35 object-cover rotate-6"
                  />
                ) : (
              ...
              <div className="text-center">
                <span
                  className={`text-[9px] font-black uppercase px-3 py-1 rounded-full ${esActivo ? "bg-green-50 text-green-600" : "bg-red-50 text-red-600"}`}
                >
                  {esActivo ? "ACTIVO" : "INACTIVO"}
                </span>
                <h3 className="font-extrabold text-[#020817] mt-3 uppercase tracking-wider text-sm">
                  {cat.nombre}
                </h3>

                {/* BADGE DEL CONTADOR DE PRODUCTOS */}
                <div className="flex items-center justify-center gap-1 mt-2 text-[10px] font-bold text-gray-500 bg-[#F8F9FF] px-3 py-1.5 rounded-lg border border-gray-100">
                  <Package size={14} className="text-[#7C3AED]" />
                  <span>{cat.cantidadProductos || 0} Productos</span>
                </div>
              </div>
            </div>
          );
      ...
      {!cargando && filtradas.length > 0 && (
        <div className="flex flex-col sm:flex-row items-center justify-between mt-4 pt-6 border-t border-gray-100 gap-4 bg-white p-4 rounded-2xl shadow-sm">
          <p className="text-[11px] font-bold text-gray-400 uppercase tracking-widest">
            Mostrando {indicePrimerRegistro + 1} -{" "}
            {Math.min(indiceUltimoRegistro, filtradas.length)} de{" "}
      ...
      {modalDesactivar && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#020817]/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-md overflow-hidden transform transition-all animate-in zoom-in-95 duration-200">
            <div
              className={`p-6 flex flex-col items-center border-b relative ${categoriaSeleccionada?.estado?.toUpperCase() === "ACTIVO" ? "bg-red-50 border-red-100" : "bg-green-50 border-green-100"}`}
            >
              <button
              ...
              <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center shadow-sm mb-4">
                <AlertCircle
                  className={`w-8 h-8 ${categoriaSeleccionada?.estado?.toUpperCase() === "ACTIVO" ? "text-red-500" : "text-green-500"}`}
                />
              </div>
              <h3
                className={`text-lg font-extrabold text-center ${categoriaSeleccionada?.estado?.toUpperCase() === "ACTIVO" ? "text-red-600" : "text-green-600"}`}
              >
                ¿
                {categoriaSeleccionada?.estado?.toUpperCase() === "ACTIVO"
                  ? "Desactivar"
                  : "Activar"}{" "}
            ...
            <div className="p-8">
              ...
              <div className="flex items-center gap-3 mt-8">
                ...
                <button
                  ...
                  disabled={
                    inputConfirmacion !== categoriaSeleccionada?.categoriaId ||
                    procesando
                  }
                ...
                >
                  {procesando
                    ? "Procesando..."
                    : `Sí, ${categoriaSeleccionada?.estado?.toUpperCase() === "ACTIVO" ? "Desactivar" : "Activar"}`}
                </button>
              </div>
            </div>
          </div>
