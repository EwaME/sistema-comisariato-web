...
export default function Inventario() {
  ...
  const isFiltroActivo =
    filtrosAplicados.categoria !== "" || filtrosAplicados.estado !== "";
  ...
  const productosFiltrados = productos.filter((prod) => {
    ...
    const matchCategoria =
      ...
      prod.categoria === filtrosAplicados.categoria;
  ...
  const confirmarCambioEstado = async () => {
    ...
    if (intentandoReactivar && stockActual <= stockMinimoCierre) {
      alert(
        `No se puede reactivar: El stock actual (${stockActual}) debe ser mayor al mínimo permitido para venta (${stockMinimoCierre}).`,
      );
      return;
    }
    ...
    try {
      ...
      setProductos(
        productos.map((p) =>
          p.id === productoSeleccionado.id ? { ...p, activo: !p.activo } : p,
        ),
      );
      cerrarModalEstado();
    } catch (error) {
    ...
    <div className="p-4 md:p-8 max-w-[1600px] mx-auto">
      <div className="mb-8">
        ...
        <p className="text-[13px] text-gray-500 mt-1 font-medium">
          Control editorial del inventario industrial
        </p>
      </div>
              ...
              <table className="w-full text-left border-collapse whitespace-nowrap">
                <thead>
                  <tr className="bg-[#F8F9FF] rounded-xl">
                    ...
                    <th className="py-4 px-4 text-[10px] font-bold text-gray-500 uppercase tracking-widest text-right">
                      Precio Contado
                    </th>
                    <th className="py-4 px-4 text-[10px] font-bold text-gray-500 uppercase tracking-widest text-right">
                      Precio Crédito
                ...
                <tbody className="text-sm font-medium text-[#020817]">
                  {productosPaginados.map((prod) => {
                    ...
                    const esCierre =
                      stockActual <= (configGlobal.StockMinimoCierre || 2);
                    const esAviso =
                      !esCierre &&
                      stockActual <= (configGlobal.StockMinimoAviso || 8);
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="py-5 px-4 text-center">
                          <span
                            className={`text-[9px] font-bold px-3 py-1.5 rounded-full uppercase tracking-widest inline-block ${estaActivo ? "bg-green-50 text-green-600" : "bg-red-50 text-red-500"}`}
                          >
                        ...
                        <td className="py-5 px-4 text-center font-bold text-gray-600">
                          {prod.cantidadVendida || 0}
                        </td>
                        <td className="py-5 px-4 text-center relative">
                          ...
                          {menuActivo === prod.id && (
                            ...
                            >
                              ...
                              <button
                                onClick={() =>
                                  !disableHabilitar && abrirModalEstado(prod)
                                }
                                disabled={disableHabilitar}
                                title={
                                  disableHabilitar
                                    ? "Stock insuficiente para habilitar"
                                    : ""
                                }
                              ...
                              <Link
                                to={`/inventario/detalle/${prod.id}`}
                                className="block w-full px-4 py-2 text-xs font-medium text-left text-gray-700 hover:bg-gray-50 transition-colors"
                              >
                                Ver detalles
                              </Link>
                            </div>
            ...
            <div className="flex flex-col gap-4">
              <div className="flex justify-between items-center p-3 rounded-xl border border-gray-100 bg-[#F8F9FF]">
                ...
                <span className="text-sm font-extrabold text-[#020817]">
                  ≤ {configGlobal.StockMinimoAviso || 8}
                </span>
              </div>
              <div className="flex justify-between items-center p-3 rounded-xl border border-red-50 bg-red-50/30">
                ...
                <span className="text-sm font-extrabold text-red-600">
                  ≤ {configGlobal.StockMinimoCierre || 2}
                </span>
              </div>
            </div>
          </div>
              ...
              {cargando ? (
                ...
                topVentas.map((prod, index) => {
                  ...
                  return (
                    <div
                      key={prod.id}
                      className={`flex items-center justify-between p-3 rounded-xl border ${esCierre ? "border-red-50 bg-red-50/30" : esAviso ? "border-amber-50 bg-amber-50/20" : "border-gray-100 hover:shadow-sm transition-shadow"}`}
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                          <span
                            className="text-xs font-bold text-[#020817] max-w-[140px] truncate"
                            title={prod.nombre}
                          >
                            {prod.nombre}
                          </span>
                      ...
                      {esCierre ? (
                        <div
                          className="w-4 h-4 rounded-full bg-red-500 flex items-center justify-center text-white text-[10px] font-bold"
                          title="¡Stock Crítico!"
                        >
                          !
                        </div>
      ...
      {modalConfirmacion && productoSeleccionado && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#020817]/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-[1.5rem] shadow-2xl w-full max-w-md overflow-hidden transform transition-all">
            <div
              className={`${productoSeleccionado.activo ? "bg-red-50 border-red-100" : "bg-green-50 border-green-100"} p-6 flex flex-col items-center border-b relative`}
            >
              <button
                onClick={cerrarModalEstado}
                className={`absolute top-4 right-4 ${productoSeleccionado.activo ? "text-red-400 hover:text-red-600" : "text-green-400 hover:text-green-600"} transition-colors`}
              >
                <X className="w-5 h-5" />
              </button>
              <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center shadow-sm mb-4">
                <AlertCircle
                  className={`w-8 h-8 ${productoSeleccionado.activo ? "text-red-500" : "text-green-500"}`}
                />
              </div>
              <h3
                className={`text-lg font-extrabold text-center ${productoSeleccionado.activo ? "text-red-600" : "text-green-600"}`}
              >
                ¿{productoSeleccionado.activo ? "Inhabilitar" : "Habilitar"}{" "}
                producto?
              </h3>
              <p
                className={`text-xs font-medium mt-1 text-center px-4 ${productoSeleccionado.activo ? "text-red-500" : "text-green-600"}`}
              >
                {productoSeleccionado.activo
            ...
            <div className="p-6">
              <p className="text-sm text-gray-600 mb-4 text-center font-medium">
                ...
                <span className="font-bold">ID del Producto</span>: <br />
