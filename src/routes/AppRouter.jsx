...
import Compras from "../pages/Manage/Inventario/Gest_Compras.jsx";
import CrearCompra from "../pages/Manage/Inventario/CrearCompra.jsx";
import DetalleCompra from "../pages/Manage/Inventario/DetalleCompra.jsx"
...
export default function AppRouter() {
  // 🔥 Definimos los grupos de acceso incluyendo al TODOLOGO en todos lados
  const TODOS_WEB = ["ADMINISTRADOR", "ACREDITADOR", "ANALISTA", "GESTOR DE INVENTARIO", "MODERADOR", "PROVEEDOR", "TODOLOGO"];
  const ADMIN_ONLY = ["ADMINISTRADOR", "TODOLOGO"];
  const ADMIN_MOD = ["ADMINISTRADOR", "MODERADOR", "TODOLOGO"];
  const STOCK_ALL = ["PROVEEDOR", "GESTOR DE INVENTARIO", "TODOLOGO"];
  const GESTOR_ONLY = ["GESTOR DE INVENTARIO", "TODOLOGO"];
  const ACREDITADOR_ONLY = ["ACREDITADOR", "TODOLOGO"];
  const ADMIN_ANALISTA = ["ADMINISTRADOR", "ANALISTA", "TODOLOGO"];
  const MODERADOR_ONLY = ["MODERADOR", "TODOLOGO"];
  ...
  return (
    <Routes>
      ...
      <Route path="/" element={ <ProtectedRoute><AdminLayout /></ProtectedRoute> }>
        ...
        {/* 1. General */}
        <Route path="dashboard" element={ <ProtectedRoute allowedRoles={TODOS_WEB}><DashboardContainer /></ProtectedRoute> } />
        <Route path="perfil" element={ <ProtectedRoute allowedRoles={TODOS_WEB}><MiPerfil /></ProtectedRoute> } />

        {/* 2. Administración y Moderación */}
        <Route path="usuarios" element={ <ProtectedRoute allowedRoles={ADMIN_MOD}><Usuarios /></ProtectedRoute> } />
        <Route path="usuarios/nuevo" element={ <ProtectedRoute allowedRoles={ADMIN_MOD}><CrearUsuario /></ProtectedRoute> } />
        ...
        {/* 7. Gestión de Stock */}
        <Route path="compras" element={ <ProtectedRoute allowedRoles={STOCK_ALL}><Compras /></ProtectedRoute> } />
        <Route path="compras/nuevo" element={ <ProtectedRoute allowedRoles={GESTOR_ONLY}><CrearCompra /></ProtectedRoute> } />
        <Route path="compras/detalle/:id" element={ <ProtectedRoute allowedRoles={STOCK_ALL}><DetalleCompra /></ProtectedRoute> } />
        <Route path="inventario" element={ <ProtectedRoute allowedRoles={GESTOR_ONLY}><Inventario /></ProtectedRoute> } />
        <Route path="inventario/nuevo" element={ <ProtectedRoute allowedRoles={GESTOR_ONLY}><CrearProducto /></ProtectedRoute> } />
        <Route path="inventario/editar/:id" element={ <ProtectedRoute allowedRoles={GESTOR_ONLY}><CrearProducto /></ProtectedRoute> } />
        <Route path="inventario/detalle/:id" element={ <ProtectedRoute allowedRoles={GESTOR_ONLY}><DetalleProducto /></ProtectedRoute> } />
        <Route path="inventario/comentarios/:id" element={ <ProtectedRoute allowedRoles={GESTOR_ONLY}><Gest_Comentarios_Producto /></ProtectedRoute> } />
        <Route path="categorias" element={ <ProtectedRoute allowedRoles={GESTOR_ONLY}><Categorias /></ProtectedRoute> } />
        <Route path="categorias/nuevo" element={ <ProtectedRoute allowedRoles={GESTOR_ONLY}><CrearCategoria /></ProtectedRoute> } />
        <Route path="categorias/editar/:id" element={ <ProtectedRoute allowedRoles={GESTOR_ONLY}><CrearCategoria /></ProtectedRoute> } />

        {/* 8. Acreditaciones */}
        <Route path="creditos" element={ <ProtectedRoute allowedRoles={ACREDITADOR_ONLY}><Creditos /></ProtectedRoute> } />
        <Route path="creditos/revision/:id" element={ <ProtectedRoute allowedRoles={ACREDITADOR_ONLY}><RevisionCredito /></ProtectedRoute> } />
        <Route path="creditos/detalle/:id" element={ <ProtectedRoute allowedRoles={ACREDITADOR_ONLY}><DetalleCredito /></ProtectedRoute> } />
        <Route path="reclamos" element={ <ProtectedRoute allowedRoles={ACREDITADOR_ONLY}><Reclamos /></ProtectedRoute> } />
        <Route path="reclamos/revision/:id" element={ <ProtectedRoute allowedRoles={ACREDITADOR_ONLY}><RevisionReclamo /></ProtectedRoute> } />

        {/* 9. Gestión de Sistema */}
        <Route path="configuraciones" element={ <ProtectedRoute allowedRoles={ADMIN_ONLY}><Configuraciones /></ProtectedRoute> } />
        <Route path="auditorias" element={ <ProtectedRoute allowedRoles={ADMIN_ANALISTA}><Auditorias /></ProtectedRoute> } />
        <Route path="reportes" element={ <ProtectedRoute allowedRoles={ADMIN_ANALISTA}><Reportes /></ProtectedRoute> } />

        {/* 10. Comunidad */}
        <Route path="sugerencias" element={ <ProtectedRoute allowedRoles={MODERADOR_ONLY}><Sugerencias /></ProtectedRoute> } />
        <Route path="guias" element={ <ProtectedRoute allowedRoles={MODERADOR_ONLY}><GuiasyAyudas /></ProtectedRoute> } />
      </Route>
